...
| **Prisma P3009 / P3018 (AlarmLevel missing)**       | Mark the failed migration as rolled back (`docker compose exec postgres psql -U command_center -d command_center -c "UPDATE ""_prisma_migrations"" SET rolled_back_at = now(), finished_at = NULL WHERE migration_name = '20251105093000_add_geofences';"`), recreate the enum (`docker compose exec postgres psql -U command_center -d command_center -c "CREATE TYPE ""AlarmLevel"" AS ENUM ('INFO','NOTICE','ALERT','CRITICAL');"`), then run `docker compose run --rm --no-deps backend pnpm --filter @command-center/backend exec prisma migrate deploy` and restart the backend.|
