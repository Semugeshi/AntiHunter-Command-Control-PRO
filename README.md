...
`scripts\setup-windows.ps1` installs Node, pnpm, and PostgreSQL via `winget` (built into Windows 10/11), creates the database, and seeds an admin login. It does **not** auto-start the app — it prints how, and writes a `Start-AntiHunter.cmd` launcher.
...
Start it by **double-clicking `Start-AntiHunter.cmd`** in the app folder, or run `pnpm AHCC` in PowerShell (non-admin is fine). Then open **http://localhost:5173** and log in with the admin credentials from setup. Update later from the app's update page, or `git pull` in the app folder.
...
> Tested with Docker Engine 25+ and Compose V2. Enable virtualization and (on Windows/macOS) configure file sharing for the repository folder.
...
2. **Clone and (optionally) set env**

   ```bash
   git clone https://github.com/TheRealSirHaXalot/AntiHunter-Command-Control-PRO.git
   cd AntiHunter-Command-Control-PRO
   cp docker/.env.example docker/.env.local   # optional: DATABASE_URL, JWT secret, admin creds
   ```

   Compose auto-loads `.env` files next to `docker-compose.yml`. To use the custom file, add `--env-file docker/.env.local` to the commands below. Skip it and the baked-in defaults apply (admin `admin@example.com` / `admin`).
...
3. **Build and start**
   ...
   - `cc_backend` runs Prisma migrations on boot (`RUN_MIGRATIONS=true`) and serves HTTP/WebSocket on `http://localhost:3000`.
...
4. **Logs / stop**
   ...
   docker compose down               # add --volumes to also drop Postgres data
   ```

<details>
<summary>Docker: upgrades, seeding, serial passthrough, overrides</summary>

- **Upgrades against an existing DB** — apply migrations before restarting:
  ```bash
  docker compose run --rm --no-deps backend \
    pnpm --filter @command-center/backend exec prisma migrate deploy
  ```
  Stuck migration (Prisma `P3009/P3018`)? See [Troubleshooting](#troubleshooting).
- **Re-run the seed** (the production image omits dev deps):
  ```bash
  ...
  "
  ```
- **Serial passthrough (Linux)** — add to the `backend` service:
  ```yaml
  devices:
  ...
  group_add:
    - dialout
  ```
  macOS/Windows Docker Desktop can't pass serial through; run the backend natively for hardware access.
- **Env overrides** — edit the `environment` block in `docker-compose.yml`, add a `docker-compose.override.yml`, or use `--env-file`. Common: `LOG_LEVEL`, `SERIAL_DEVICE`, `ALLOW_FOREVER`, mail settings.
- **Skip migrations** — set `RUN_MIGRATIONS=false` if you deploy schema externally.
- **Live reload** — the Docker workflow runs compiled artefacts; for development use the [pnpm workflow](#running-the-stack) with hot reload.
</details>
