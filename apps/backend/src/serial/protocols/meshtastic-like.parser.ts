...
const STATUS_REGEX = /^(?<node>[A-Za-z0-9_\-]+):\s*STATUS:\s*Mode:(?<mode>[A-Za-z0-9+]+)\s+Scan:(?<scan>[A-Za-z]+)\s+Hits:(?<hits>\d+)\s+Unique:(?<unique>\d+)\s+Temp:\s*(?<tempC>[0-9.]+).?C\s*\/\s*(?<tempF>[0-9.]+).?F\s+Up:(?<uptime>[0-9:]+)/i;
const RTC_SYNC_REGEX = /^(?<node>[A-Za-z0-9_\-]+):\s*RTC_SYNC:\s*(?<ts>\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2})\s*UTC/i;
const TIME_SYNC_REGEX = /^(?<node>[A-Za-z0-9_\-]+):\s*(?<kind>TIME_SYNC_REQ|TIME_SYNC):(?!$)(?<body>.*)$/i;
const SETUP_MODE_REGEX = /^(?<node>[A-Za-z0-9_\-]+):\s*SETUP_MODE:\s*Auto-erase activates in\s*(?<secs>\d+)s/i;
const CONFIG_ACK_REGEX = /^(?<node>[A-Za-z0-9_\-]+):\s*CONFIG_ACK:(?<type>[A-Z_]+):(?<value>.+)$/i;
const OP_ACK_REGEX = /^(?<node>[A-Za-z0-9_\-]+):\s*(?<kind>SCAN|DEVICE_SCAN|DRONE|DEAUTH|RANDOMIZATION|BASELINE)_ACK:(?<status>[A-Z_]+)/i;
...
const BASELINE_STATUS_REGEX = /^(?<node>[A-Za-z0-9_\-]+):\s*BASELINE_STATUS:\s*Scanning:(?<scanning>YES|NO)\s*Established:(?<est>YES|NO)\s*Devices[=:](?<devices>\d+)\s*Anomalies[=:](?<anomalies>\d+)\s*Phase1:(?<phase>ACTIVE|COMPLETE)/i;
const ERASE_ACK_REGEX = /^(?<node>[A-Za-z0-9_\-]+):\s*ERASE_ACK:(?<status>STARTED|COMPLETE|CANCELLED|FAILED)/i;
...
const GENERIC_NODE_LINE_REGEX = /^(?:\[(?<tag>[A-Z_]+)\]\s*)?(?<node>[A-Za-z0-9_\-]+):\s*(?<body>.+)$/;
...
export class MeshtasticLikeParser implements SerialProtocolParser {
  ...
  reset(): void {
    ...
    private parseBinary(raw: string): SerialParseResult[] {
        ...
        let packet: any;
        try {
            packet = pb.Mesh.MeshPacketSchema.fromBinary(Buffer.from(raw, 'binary'));
        }
        catch {
            return [{ kind: 'raw', raw }];
        }
        ...
        if (decoded.portnum === pb.Portnums.PortNum.LOCATION_APP && decoded.payload) {
            try {
                const position = pb.Telemetry.PositionSchema.fromBinary(decoded.payload);
                if (position.latitudeI != null && position.longitudeI != null) {
                    events.push(this.toTelemetry(nodeId, position.latitudeI / 1e7, position.longitudeI / 1e7, raw, position.time ? new Date(position.time * 1000).toISOString() : undefined));
                    return events;
                }
            }
            catch {
        ...
        if (decoded.portnum === pb.Portnums.PortNum.TELEMETRY_APP && decoded.payload) {
            try {
                const telemetry = pb.Telemetry.TelemetrySchema.fromBinary(decoded.payload);
                events.push({
                    kind: 'alert',
                    level: 'INFO',
                    category: 'telemetry',
        ...
        if (decoded.payload) {
            ...
            if (text) {
                const forwarded = this.parseText(text);
                if (forwarded) {
                    return forwarded.map((event) => ({ ...event, raw }));
                }
                events.push({
                    kind: 'alert',
                    level: 'INFO',
    ...
    private parseText(line: string, forwarded = false): SerialParseResult[] | null {
        ...
        if (forwardedMatch?.groups) {
            ...
            const forwardedResults = this.parseText(forwardedLine, true);
            if (forwardedResults) {
        ...
        if (deviceMatch?.groups) {
            ...
            const band = deviceMatch.groups.band
                ? resolveDeviceType(deviceMatch.groups.band)
                : undefined;
    ...
    if (gpsSimpleMatch?.groups) {
      ...
      if (lat != null && lon != null) {
        ...
        if (ackMatch?.groups) {
            ...
            results.push(this.buildAckEvent(nodeId, `${ackMatch.groups.kind}_ACK`, ackMatch.groups.status, line));
            return this.deliverOrRaw(results, line);
        }
    ...
    private shouldDeduplicateStatus(nodeId: string, message: string): boolean {
        ...
        if (cacheEntry && cacheEntry.message === message && now - cacheEntry.timestamp < STATUS_DEDUP_MS) {
            return true;
        }
    ...
    private deliverOrRaw(results: SerialParseResult[], line: string): SerialParseResult[] {
        ...
        return [{ kind: 'raw', raw: line } as SerialRawFrame];
    }
    ...
    private buildAckEvent(nodeId: string, ackType: string, status: string, raw: string): SerialCommandAck {
        return {
            kind: 'command-ack',
            nodeId,
